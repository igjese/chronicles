...
func change_state(new_state):
    ...
    if current_state != old_state:
        if old_state and old_state.has(EXIT): old_state[EXIT].call()
        if current_state and current_state.has(ENTER): current_state[ENTER].call()
    
func process(delta):
    if current_state and current_state.has(PROCESS): current_state[PROCESS].call(delta)
